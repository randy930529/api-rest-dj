    ...
    try {
      ...
      const expensesPDGT =
        this.fiscalYear?.supportDocuments.filter(
          (val) =>
            val.type_document === "g" && val.element?.group?.startsWith("pd")
        ) || [];

      const {
        sumaTotalExpensesWithoutDocument,
        sumaTotalExpensesWithDocument,
      } = expensesPDGT.reduce(
        (sumaTotal, val) => {
          if (!val.document) {
      ...
      console.error("SECTION_ERROR: ", error.message);
      return {};
    }
  }
}
